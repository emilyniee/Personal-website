...
export default function ExperienceCard({ company, position, timeline, readMore, imageSrc }: ExperienceCardProps) {
  return (
    <article className="flex flex-row p-4 md:p-6 lg:p-8 items-center gap-3 md:gap-4 border-b border-coffee-200 last:border-b-0">
      <div className="w-16 h-16 md:w-24 md:h-24 lg:w-28 lg:h-28 flex-shrink-0">
        <Image
          src={imageSrc}
          alt={`${company} logo`}
          width={100}
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs md:text-sm text-black mt-1">{timeline}</p>
        <a
          className="text-xs md:text-sm text-black hover:underline inline-block mt-1"
          href={readMore}
